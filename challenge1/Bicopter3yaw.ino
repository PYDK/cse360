...
void loop() {
  ...
  if (controls[0] == 0) {
    ...
    outputs[0] = 0;
    outputs[1] = 0;
    outputs[2] = 80; // change if you are not using upwards facing motor/servos
    outputs[3] = 80; // change if you are not using upwards facing motor/servos
  ...
  if (PDterms.yawEn) {
    ...
    difference = min(left, right);
    float newYawRate = 0;
    if(yawrate < 0) {
      newYawRate = -yawrate;
    }
    // PID in z: the output is the force to maintain the altitude
    tz = difference * PDterms.kpyaw - newYawRate * PDterms.kdyaw;
    tz = constrain(tz, 0, 0.2);
  }
  ...
  fz = constrain(fz, 0, 1000);
  float m1 = sqrt(sq(0.05)+sq(fz));  // motor 1
  float m2 = sqrt(sq(0.05)+sq(fz));  // motor 2
  ...
  float t1 = 90;  // servo 1
  float t2 = 90;  // servo 2
  t1 += degrees(atan2(fz,0.05));
  t2 += degrees(atan2(fz,0.05));


  m1 = constrain(m1, 0, 1);
  m2 = constrain(m2, 0, 1);
  t1 = constrain(t1, 40, 150);
  t2 = constrain(t2, 40, 150);
  if (micros() - printTime > 500000){
    Serial.printf("%f || %f || %f ||%f ||%f || %f\n", altitude, t1, t2, m1, m2, yaw);
    printTime = micros();
  }
